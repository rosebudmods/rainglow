...
public class CustomModeScreen extends RainglowScreen {
    ...
    private final SpruceBooleanOption[] colourToggles = new SpruceBooleanOption[EntityColour.values().length];
    private final boolean[] toggleStates = new boolean[EntityColour.values().length];
    ...
    public CustomModeScreen(@Nullable Screen parent) {
        ...
        for (int i = 0; i < EntityColour.values().length; i ++) {
            final EntityColour colour = EntityColour.values()[i];
            final int index = i;
        ...
        this.saveOption = SpruceSimpleActionOption.of(Rainglow.translatableTextKey("config.save"),
                buttonWidget -> {
                    List<EntityColour> newCustom = new ArrayList<>();

                    for (int i = 0; i < EntityColour.values().length; i ++) {
                        if (toggleStates[i]) {
                            newCustom.add(EntityColour.values()[i]);
                        }
                    }
    ...
    protected void init() {
        ...
        for (int i = 0; i < EntityColour.values().length; i += 2) {
            SpruceOption secondToggle = null;
            if (i + 1 < EntityColour.values().length) {
