...
public class CustomModeScreen extends RainglowScreen {
    ...
    private final SpruceBooleanOption[] colourToggles = new SpruceBooleanOption[EntityGlowColour.values().length];
    private final boolean[] toggleStates = new boolean[EntityGlowColour.values().length];
    ...
    public CustomModeScreen(@Nullable Screen parent) {
        ...
        for (int i = 0; i < EntityGlowColour.values().length; i ++) {
            final EntityGlowColour colour = EntityGlowColour.values()[i];
            final int index = i;
        ...
        this.saveOption = SpruceSimpleActionOption.of(Rainglow.translatableTextKey("config.save"),
                buttonWidget -> {
                    List<EntityGlowColour> newCustom = new ArrayList<>();

                    for (int i = 0; i < EntityGlowColour.values().length; i ++) {
                        if (toggleStates[i]) {
                            newCustom.add(EntityGlowColour.values()[i]);
                        }
                    }
    ...
    protected void init() {
        ...
        for (int i = 0; i < EntityGlowColour.values().length; i += 2) {
            SpruceOption secondToggle = null;
            if (i + 1 < EntityGlowColour.values().length) {
